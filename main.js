  ...
  AppData.districts.forEach((district, index) => {
    ...
    card.innerHTML = `
      <div class="flex items-start justify-between mb-4">
        ...
        <span class="status-badge status-${district.status.toLowerCase()}">${district.status}</span>
      </div>
      
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-4 text-center">
          <p class="text-2xl font-bold gradient-text">${formatNumber(district.totalUnits)}</p>
        ...
        <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-4 text-center">
          <p class="text-2xl font-bold text-amber-700">${district.criticalTypes}</p>
          <p class="text-xs text-gray-600">Critical Types</p>
        </div>
      </div>
      
      <button class="w-full btn btn-secondary text-sm" onclick="showToast('info', 'Viewing details for ${district.name}')">
...
function handleBloodRequest(e) {
  ...
  openModal('successModal');
  document.getElementById('successMessage').textContent = 
    `Blood request from ${hospital} has been submitted successfully. Nearby blood banks have been notified.`;
